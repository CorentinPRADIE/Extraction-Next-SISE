...
def menu():
    ...
    dim_projets_ready = sum(1 for fichier in os.listdir(PATH_DIR_DIM_PROJETS) if fichier.endswith('.csv')) == 1
    fact_financiers_enr_ready = sum(1 for fichier in os.listdir(PATH_DIR_FACT_ENR) if fichier.endswith('.csv')) == 1

    st.write(f"dim_projets         : {' :white_check_mark: le fichier est bien déposé' if dim_projets_ready else ' :x: Verifier que le fichier est bien déposé et qu il n y ait qu un seul fichier'}")
    st.write(f"fact_financiers_enr : {' :white_check_mark: le fichier est bien déposé' if fact_financiers_enr_ready else ':x: Verifier que le fichier est bien déposé et qu il n y ait qu un seul fichier'}")
    ...
    with st.expander("Fichiers de sortie :"):
        ...
        st.session_state['FIN_CONSO'] = st.checkbox("Une extraction du Financier Consommation (sans les montants à 0)", True)
        st.session_state['COFI'] = st.checkbox("Une extraction du Financier Cofinancement (sans les montants à 0)", True)
        st.session_state['BRUTE'] = st.checkbox("Une extraction des engagements avec les montants à 0", False)
        st.write('#### Scripts :')
    ...
    return dim_projets_ready, fact_financiers_enr_ready

def main():
    
    dim_projets_ready, fact_financiers_enr_ready = menu()
    ...
    if start:
        ...
        if not OPERATEURS_SELECTED:
        ...
        elif not fact_financiers_enr_ready:
            st.write(f"Veillez a ce que la fact_financiers_enr.csv soit bien déposé dans le dossier {PATH_DIR_FACT_ENR}")
        elif not dim_projets_ready:
            st.write(f"Veillez a ce que la dim_projets.csv soit bien déposé dans le dossier {PATH_DIR_DIM_PROJETS}")
        else:
            ...
            with st.status("Chargement...", expanded=True) as status:
                ...
                os.system(f'explorer "{os.path.join(os.getcwd(),DIR_PARENT)}"')
...
def load_fact_enr_parquet_per_action(PATH_FACT_ENR_parquet, dim_projets, operateur_id, action_id, engagements_avec_montant_nulles=False):
    ...
    if engagements_avec_montant_nulles:
...
def write_actions(PATH_FACT_ENR_parquet, dim_projets, DIR_PARENT, OPERATEURS_SELECTED):
    ...
    for num_ope, operateur_id in enumerate(OPERATEURS):
        ...
        if BRUTE:
            BRUTE_DIR = f'{DIR_PARENT}/Financier Consommation - Engagments avec montants nulles/{operateur_id}'
        ...
        for num_act, action_id in enumerate(action_ids):
            ...
            if BRUTE:
                fact_enr_parquet_action = load_fact_enr_parquet_per_action(PATH_FACT_ENR_parquet, dim_projets, operateur_id, action_id, engagements_avec_montant_nulles=True)
                df_brute = fact_enr_parquet_action.sort_values(['projet_id', 'period_id'], ascending=False, na_position='last')
                df_brute.to_excel(BRUTE_DIR + file_name, sheet_name='Eng ac montants nulles', index=False)
